...
figma.ui.onmessage = async(msg) => {
  ...
  if (msg.type === 'clone') {
    ...
    for( const d of msg.selop){console.log(d.type)}
      ...
      for(const c of msg.data){        
        for( const d of msg.selop){
          ...
          if(node){
            ...
            if(d.type=='txt'){
              try{
              await updateText(node,c[d.value])
              }
              catch(e){
                console.log(e)
              }
  ...
  if(msg.type==='newimage'){
    ...
    for(const g of node.fills){
      if(g.type=='IMAGE'){
        ...
        const newimgfill={...g,imageHash:imgfill.hash}
...
function handleselection(){
  ...
  while (selected.length != 0) {
    ...
    if (l1.type == 'TEXT' && prefixCheck(l1,prefix)) newselect.push({name:l1.name,id:l1.id, type:"txt"})
    else
    if ((l1.type == 'RECTANGLE' || l1.type == 'ELLIPSE' || l1.type == 'VECTOR') && hasImageFill(l1) && prefixCheck(l1,prefix)){
      newselect.push({name:l1.name,id:l1.id, type:"img"})
    }
    else
    if (l1.type == 'FRAME' || l1.type == 'GROUP' || l1.type == 'COMPONENT' || l1.type=='INSTANCE'){
...
async function updateText(node,txt){
  if(node.type=='TEXT'){
    try{
...
async function updateFill(pid,id,url){  
  try{
